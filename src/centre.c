...
int ajouter_centre(Centre c) {
    ...
    if (f != NULL) {
        while (fgets(ligne, sizeof(ligne), f) != NULL && compteur < MAX_LIGNES) {
            // Vérifier que la ligne n'est pas vide
            if (strlen(ligne) < 3) continue;
            
            // Parser la ligne - vérifier qu'on a bien lu 10 champs
            ...
            if (scan_result == 10) {  // ✅ CORRECT - vérifier le nombre de champs lus
                compteur++;
                if (tmp.id == c.id) {
                    id_existe = 1;
                    break;
    ...
    return 0; // Échec - impossible d'ouvrir le fichier
}
...
void supprimer_centre(int id) {
    ...
    if (f != NULL && temp != NULL) {
        while (fgets(ligne, sizeof(ligne), f) != NULL) {
            ...
            if (scan_result == 10 && c.id != id) {
                // Garder seulement les lignes dont l'ID est différent
                fprintf(temp, "%s", ligne);
            }
        }
